...
namespace ion
{
	...
	public:
		 ...
		 */
		template<typename ReturnValue, typename ScopeCallback>
		static ReturnValue scoped(const std::filesystem::path& path, ScopeCallback scope)
		{
			...
			try
			{
				ReturnValue returnValue = scope(logger);
				dispose();
				return returnValue;
			}
		 ...
		 */
		template<typename ReturnValue, typename ScopeCallback>
		static ReturnValue scoped(ScopeCallback scope)
		{
			auto path = std::filesystem::current_path() / "logs";
			return scoped<ReturnValue, ScopeCallback>(path, scope);
		}
